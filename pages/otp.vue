<template>
    <div class="mx-auto w-full">
      <div class="grid grid-cols-12">
        <div class="lg:col-span-6 col-span-12 lg:mt-20 mt-2">
          <div class="w-full h-full">
            <div class="flex items-center justify-start lg:px-12 px-2">
              <a href="javascript:void(0)" @click="goBack()">
                <button class="p-2 bg-red-500 rounded-lg">
                  <svg
            ...
            <div class="flex items-center justify-center mt-1 py-2">
              <form  @submit.prevent="submitForm()"
                class="relative text-sm rounded-full"
              >
              <div class="relative mt-2 text-sm border border-slate-200 dark:border-gray-700 rounded-full">
                <input
                  class="w-full text-white p-4 md:pl-32 pl-20 dark:bg-[#1a161e] rounded-full"
                  type="text"
              ...
              <div class="flex items-center justify-center mt-3">
                <span class="text-white">زمان باقی مانده : {{ formattedTime }}</span>
              </div>
  
                <div class="flex items-center justify-center py-5">
                    <button
                        type="submit" v-if="registerLoading == false"
                        class="bg-red-600 px-10 text-white py-4 w-full rounded-full tracking-wide font-semibold font-display focus:outline-none focus:shadow-outline hover:bg-red-800 shadow-lg"
                    >
                        {{ route.query.action == 'login' ? 'ورود' : 'عضویت' }}   
                    </button>
                    <button
                        type="button" v-if="registerLoading == true" disabled 
                        class="bg-red-600 text-white px-10 py-4 w-full rounded-full tracking-wide font-semibold font-display focus:outline-none focus:shadow-outline hover:bg-red-800 shadow-lg"
                    >
        ...
        <div
          class="col-span-6 lg:flex dark:bg-[#1a161e] bg-slate-100 hidden items-center justify-center"
        >
